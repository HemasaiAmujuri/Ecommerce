...
const updateCartItem = async (req, res) => {
  try {
    ...
    const cartIndex = cartProducts.findIndex(
      (item) => Number(item.productId) === Number(id) && item.deletedAt === null
    );
...
const deleteCartItem = async (req, res) => {
  try {
    ...
    const cartProducts = loadCart();
    const cartIndex = cartProducts.findIndex((item) => Number(item.productId) === Number(id) && item.deletedAt === null);
